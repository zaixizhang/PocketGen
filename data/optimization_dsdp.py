...
def calculate_vina(pro_path, lig_path, output=False):
    size_factor = 1.
    buffer = 5.
...
def extract_number(path):
    """
    Extracts the numerical value from a file that contains a specific formatted line.

    :param path: str, The path to the file.
    :return: float or None, The extracted numerical value or None if there's an error.
    """
    try:
        ...
        with open(path, 'r') as file:
            ...
            for line in file:
                # Split the line into parts assuming space is the separator.
                parts = line.strip().split()  # this will remove leading and trailing whitespaces before splitting

                # If there are two parts and the second one is a float, we take it.
                if len(parts) == 2:
                    try:
                        # Attempt to convert the second part into a float (the numerical value you want).
                        number = float(parts[1])
                        return number
                    except ValueError:
                        # Handle the case where the conversion to float fails.
                        print(f"Error: {parts[1]} is not a valid float.")
                        return None
                else:
...
def calculate_dsdp(pro_path, lig_path):
    openmm_relax(pro_path)
    relax_sdf(lig_path)
    ligand_pdbqt = './data/tmp/complex/complex_ligand.pdbqt'
    protein_pqr = './data/tmp/complex/' + 'pro.pqr'
    protein_pdbqt = './data/tmp/complex/complex_protein.pdbqt'
    lig = PrepLig(lig_path, 'sdf')
    lig.addH()
    lig.get_pdbqt(ligand_pdbqt)
    ...
    args = [
        ...
        '--dataset_name', 'PDBBind_time_split_dataset',
        '--site_path', './data/DSDP_op/site_output/',
        '--exhaustiveness', '384',
        '--search_depth', '40',
        '--top_n', '1',
        '--out', './data/DSDP_op/docking_results/',
        '--log', './data/DSDP_op/docking_results/'
    ]
    cmd = [script_path] + args
    result = subprocess.run(cmd, check=True, text=True, capture_output=True, shell=True)
    print(f"Output:\n{result.stdout}")
    log_file = 'complex_out.log'
    score = extract_number(os.path.join('./data/DSDP_op/docking_results/', log_file))
    return score
...
if __name__ == '__main__':
    ...
    for i in tqdm(range(len(index))):
        ...
        try:
            ...
            dir_name = os.path.join('./data/DSDP_op', pdb_id)
            ...
            for j in range(optimization_steps):
                ...
                datalist = pdb2data(lig_path, pro_path, seq, full_seq_idx, r10_idx, protein_edit_residue, protein_filename,
                ...
                for k in range(10):
                    try:
                        score = calculate_vina('./data/DSDP_op/po1/' + str(k) + '_whole.pdb', './data/DSDP_op/po1/' + str(k) + '.sdf', output=True)
                    except:
                        score = 0.
                ...
                pro_path = './data/DSDP_op/po1/' + str(argmin) + '.pdb'
                lig_path = './data/DSDP_op/po1/' + str(argmin) + '.sdf'
                whole_pro_path = './data/DSDP_op/po1/' + str(argmin) + '_whole.pdb'
                docked_path = './data/DSDP_op/po1/' + str(argmin) + '_whole_docked.pdbqt'
                ...
                seq = ''.join(seq_list)
            torch.save(record, './data/DSDP_op/po1/record.pt')
