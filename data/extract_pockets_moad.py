...
def process_item(item, args):
    try:
        ...
        r10_idx, r10_residues = protein.query_residues_ligand(ligand, args.radius, selected_residue=None, return_mask=False)
        ...
        return pocket_fn, ligand_fn, item[0], 0., seq, full_seq_idx, r10_idx  # item[0]: original protein filename; item[2]: rmsd.

    except Exception:
        print('Exception occurred.', item)
        return None, item[1], item[0], 0., None, None, None
...
if __name__ == '__main__':
    ...
    for split in ['train', 'val', 'test']:
        ...
        pocket = []
        for item_pocket in tqdm(pool.imap_unordered(partial(process_item, args=args), index[split]), total=len(index[split])):
